...
public class MPIPageRank {
    ...
    while((cnter < iterations) && (delta < dif[0])) {
        dif[0] = 0.0;
        cnter++;

        double total[] = new double[totalUrls];
        ...
        for(Integer page: localAdjMat.keySet()){
            ...
            if(outdegree == 0.0){
                for(int k = 0; k < globalKeys.length; k++){
                    total[k] += globalRanks[getRankIndex(page)]/(double)totalUrls;
                }
            }
            else{
                for(int k: outlinks)
                    {
                    total[getRankIndex(k)] += globalRanks[getRankIndex(page)]/outdegree;
                    }
                }
        }

        MPI.COMM_WORLD.Allreduce(total, 0, sum, 0, totalUrls, MPI.DOUBLE, MPI.SUM);
        if(rank == 0) {
            for(int i = 0; i < totalUrls; i++) {
